...
def parse_args():
    ...
    parser.add_argument("--VAE-lr", type=float, default=3e-4, help="the learning rate of VAE")
    parser.add_argument("--num-pre-epoches", type=int, default=100, help="the number of pretrain epoches")
    parser.add_argument("--batch-size-vae", type=int, default=512, help="the batch size used to train the networks")
...
if __name__=="__main__":
    ...
    for global_step in range(args.max_step):
        ...
        if global_step < args.explore_step:
            ...
            with torch.no_grad():
                start = time.time()
                input_vector,_,_ = encoder(torch.FloatTensor(obs).to(device))
                actions, _, _ , _= actor.get_action(input_vector)
                end = time.time()
                policy_forward_time = end - start
                if args.wandb_track and global_step % 100 == 0:
                    wandb.define_metric("z", step_metric="Global_step")
                    wandb.log({
                        "z": wandb.Histogram(input_vector.detach().cpu().numpy()),
                        "Global_step": global_step
                    })
        ...
        if global_step > args.explore_step:
            ...
            obs_vectors, _, _ = encoder(batch['obs'])
            next_obs_vectors, _, _ = encoder(batch['next_obs'])
