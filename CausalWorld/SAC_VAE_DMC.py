...
if __name__=="__main__":
    ...
    #actor.load_state_dict(torch.load('Models/pixel_actor_DMC.pth'))
    #Q_net.load_state_dict(torch.load('Models/pixel_Q_DMC.pth'))
    ...
    for global_step in range(args.max_step):
        ...
        if global_step % args.save_frequency == 0:
            torch.save(actor.state_dict(), 'Models/VAE_actor_DMC.pth')
            torch.save(Q_net.state_dict(), 'Models/VAE_Q_DMC.pth')
            torch.save(encoder.state_dict(), 'Models/encoder_DMC.pth')
            torch.save(decoder.state_dict(), 'Models/decoder_DMC.pth')
