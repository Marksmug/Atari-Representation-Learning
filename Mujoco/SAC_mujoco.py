# the auto tuning alpha function is NOT checked
...
def parse_args():
    ...
    parser.add_argument("--auto-alpha", type=bool, default=False, help="the flag that using auto temperature tuning")
...
class replayBuffer():
    ...
    def store(self, obs, act, r, next_obs, done):
        ...
        # handel the initial state of gymnaisum environment: tuple(array,{})
    ...
    def sample(self, batch_size=32):
        ...
        # the reward is rescaled accoriding to the original paper
        batch = dict(
            obs = self.obs_buffer[idx],
            act = self.act_buffer[idx],
            r = self.r_buffer[idx]*5,
...
def sac_update(global_step, rep_buffer, actor, Q_net1, Q_net2, Q_target1, Q_target2, alpha):
    ...
    if global_step % args.policy_frequency == 0:
        ...
        for _ in range(args.policy_frequency):
            ...
            if args.auto_alpha:
                with torch.no_grad():
                    _, log_probs, _ = actor.get_action(batch["obs"])
...
if __name__=="__main__":
    ...
    for global_step in range(args.max_step):
        ...
        # below is used to record the episodic reward and length exclusively for gymnaisum environment
        if "final_info" in infos:
            for info in infos["final_info"]:
                print(f"episode_num={episode_num}, global_step={global_step}, episodic_return={info['episode']['r']}")
                episode_num += 1
                if args.wandb_track:
